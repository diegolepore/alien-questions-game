<template>
  <div class="contain">
    <GamestateStart v-if="uiState === 'start'">
      ...
      <button :disabled="!characterInput" @click="pickCharacter">Pick your character</button>
    </GamestateStart>
    ...
    <section v-if="uiState === 'characterChosen'">
      <svg viewBox="0 -180 1628 1180" class="main">
        <defs>
          <clipPath id="bottom-clip">
            <rect class="bottom-clip-path" x="1131.5" :y="bottomClipPathY" width="406" height="1000" />
          </clipPath>
          <clipPath id="top-clip">
            <rect class="top-clip-path" x="1131.5" :y="topClipPathY" width="406" height="473" />
          </clipPath>
        </defs>
        ...
        <component class="character-clip" :is="character"></component>
      ...
      <div class="zombietalk">
        <p v-for="character in characterChoices.shuffle()" :key="character">
          <button @click="selectAnswer(character)">
    ...
    <GamestateFinish @on-start-over="resetData" v-if="uiState === 'won' || uiState === 'lost'" />
...
export default {
  ...
  methods: {
    pickCharacter() {
      this.$store.commit('pickCharactaer', this.characterInput)
      this.$store.commit('updateUiState', 'characterChosen')
    },

    selectAnswer(char) {
      this.$store.commit('selectAnswer', char)
    },
